...
class NeoForgeLuxSchedulerTest {

    private lateinit var scheduler: NeoForgeLuxScheduler
    private lateinit var mockTickEvent: ServerTickEvent.Post

    @BeforeEach
    fun setup() {
        scheduler = NeoForgeLuxScheduler()
        // Mock the required NeoForge event object
        mockTickEvent = mock<ServerTickEvent.Post>()
    }

    /**
     * Helper function to manually trigger the public 'onServerTick' method
     * so we can precisely simulate the Minecraft server game loop.
     */
    private fun simulateTick() {
        scheduler.onServerTick(mockTickEvent)
    }
