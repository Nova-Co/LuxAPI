...
/**
 * Fabric-specific implementation of the PlayerManager.
...
class FabricPlayerManager(private val server: MinecraftServer) : PlayerManager {
    ...
    fun login(player: net.minecraft.server.level.ServerPlayer): LuxPlayer {
        val luxPlayer = FabricLuxPlayer(player)
    ...
    fun updatePlayerInstance(player: net.minecraft.server.level.ServerPlayer) {
        val cached = cachedPlayers[player.uuid] as? FabricLuxPlayer
    ...
    override fun getOnlinePlayers(): List<LuxPlayer> {
        return server.playerList.players.map { getPlayer(it.uuid) ?: FabricLuxPlayer(it) }
    }
}